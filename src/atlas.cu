...
static void get_chart_connectivity(
    ...
    init_chart_adj_kernel<<<(M + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        mesh.vertices.ptr,
        mesh.faces.ptr,
    ...
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        ...
        cu_raw_lengths,
        cu_sorted_lengths,
        M, 0, sizeof(*(mesh.atlas_chart_adj.ptr)) * 8, cumesh::torch_cuda_stream()));
    ...
    get_chart_edge_cnt_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        mesh.atlas_chart_adj.ptr,
        mesh.atlas_chart_adj_length.ptr,
        E,
    ...
    mesh.atlas_chart2edge_cnt.zero();
    get_chart_edge_adjacency_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        mesh.atlas_chart_adj.ptr,
        E,
...
void compute_chart_normal_cones(
    ...
    arange_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        faces_ids,
        F
    );
    CUDA_CHECK(cudaGetLastError());
    ...
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        ...
        F, 0, sizeof(*(mesh.atlas_chart_ids.ptr)) * 8, cumesh::torch_cuda_stream()));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceRadixSort::SortPairs(
        mesh.cub_temp_storage.ptr, temp_storage_bytes,
        mesh.atlas_chart_ids.ptr, sorted_chart_ids,
    ...
    CUDA_CHECK(cub::DeviceSegmentedReduce::Sum(
        ...
        C,
        cu_chart_offsets, cu_chart_offsets + 1, cumesh::torch_cuda_stream()));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSegmentedReduce::Sum(
        mesh.cub_temp_storage.ptr, temp_storage_bytes,
        cu_sorted_face_areas, mesh.atlas_chart_areas.ptr,
    ...
    CUDA_CHECK(cub::DeviceSegmentedReduce::Reduce(
        ...
        Float3Add(),
        make_float3(0.0f, 0.0f, 0.0f), cumesh::torch_cuda_stream()));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceSegmentedReduce::Reduce(
        mesh.cub_temp_storage.ptr, temp_storage_bytes,
        cu_sorted_face_normals, cu_chart_normals,
        ...
        Float3Add(),
        make_float3(0.0f, 0.0f, 0.0f), cumesh::torch_cuda_stream()));
    normalize_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        cu_chart_normals,
        C
    );
    CUDA_CHECK(cudaGetLastError());
    ...
    normal_diff_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        cu_chart_normals,
        cu_sorted_face_normals,
        sorted_chart_ids,
        F,
    ...
    update_normal_cones_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
...
static void reassign_chart_ids(
    ...
    do {
        ...
        // Hook
        hook_edges_if_same_chart_kernel<<<(M+BLOCK_SIZE-1)/BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
            mesh.manifold_face_adj.ptr,
            mesh.atlas_chart_ids.ptr,
            M,
            reinterpret_cast<int*>(mesh.temp_storage.ptr),
...
void construct_chart_mesh(
    ...
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
        ...
        mesh.atlas_num_charts + 1, cumesh::torch_cuda_stream()));
    mesh.cub_temp_storage.resize(temp_storage_bytes);
    CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
        mesh.cub_temp_storage.ptr, temp_storage_bytes,
        cu_chart_size, mesh.atlas_chart_faces_offset.ptr,
    ...
    expand_chart_ids_and_vertex_ids_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        cu_sorted_chart_ids,
        cu_sorted_face_idx,
        mesh.faces.ptr,
        F,
    ...
    unpack_vertex_ids_kernel<<<(new_num_vertices + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        cu_inverse_pack,
        new_num_vertices,
    ...
    unpack_faces_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        cu_pack,
        F,
...
void CuMesh::compute_charts(
    ...
    for (int i = 0; i < global_iterations; i++) {
        while (true) {
            ...
            compute_chart_adjacency_cost_kernel<<<(E + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
            ...
            propagate_cost_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
            ...
            arange_kernel<<<(C + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
        ...
        for (int j = 0; j < refine_iterations; j++) {
            compute_chart_normal_cones(*this);
            this->temp_storage.resize(F * sizeof(int));
            refine_charts_kernel<<<(F + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, cumesh::torch_cuda_stream()>>>(
                this->atlas_chart_normal_cones.ptr,
